...
describe('AuraCursor', () => {
  ...
  const rafTimeouts: Map<number, NodeJS.Timeout> = new Map();

  const setupRAFMock = () => {
    rafId = 0;
    rafCallbacks.clear();
    rafTimeouts.forEach((timeout) => clearTimeout(timeout));
    rafTimeouts.clear();
    ...
    (
      window as unknown as {
        requestAnimationFrame: typeof rafMock;
        cancelAnimationFrame: typeof cafMock;
      }
  ...
  beforeEach(() => {
    ...
    ).__AURA_CURSOR_FORCE_DESKTOP__ = true;
